  
import os
import glob
import pandas as pd
import xml.etree.ElementTree as ET
import argparse
import sys
# taken from https://github.com/datitran/raccoon_dataset

def xml_to_csv(dataset_dir):
    xml_list = []
    for folder in os.listdir(dataset_dir):
        xmls = glob.glob(dataset_dir+"/"+folder+"/*.xml")
        for xml_file in xmls:
            tree = ET.parse(xml_file)
            root = tree.getroot()
            for member in root.findall('object'):
                print(member[4][3].text)
                value = (xml_file.replace("xml","png"),
                        int(root.find('size')[0].text),
                        int(root.find('size')[1].text),
                        member[0].text,
                        int(member[4][0].text),
                        int(member[4][1].text),
                        int(member[4][2].text),
                        int(member[4][3].text)
                        )
                xml_list.append(value)
    column_name = ['filename', 'width', 'height', 'class', 'xmin', 'ymin', 'xmax', 'ymax']
    xml_df = pd.DataFrame(xml_list, columns=column_name)
    return xml_df


def main(): # this is the part we change to work with our setup
    # Initiate the parser
    parser = argparse.ArgumentParser()

    # Add long and short argument
    parser.add_argument("--dataset_dir", help="dataset dir path")

    # Read arguments from the command line
    args = parser.parse_args()

    # Check for --width
    DATASET_DIR = ""
    if args.dataset_dir:
        DATASET_DIR = args.dataset_dir
        print(args.dataset_dir)
    else:
        print("specify the dataset dir path")
        sys.exit(1)
    xml_df = xml_to_csv(DATASET_DIR+"/train")
    xml_df.to_csv("train_labels.csv", index=None)
    xml_df = xml_to_csv(DATASET_DIR+"/val")
    xml_df.to_csv("val_labels.csv", index=None)
    print('Successfully converted xml to csv.')


main()
